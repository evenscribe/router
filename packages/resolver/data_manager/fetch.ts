...
const openrouter = (path: string, category: string, order: string) =>
  Effect.gen(function* () {
    const response = yield* Effect.tryPromise({
...
const generateOpenRouterFetches = (path: string, pairs: string[][]) => {
  return pairs.map(([category, order]) => openrouter(path, category!, order!));
};

const makePaths = (path: string) =>
  Effect.gen(function* () {
    const fs = yield* FileSystem;
    const lastFetchPath = `${path}/.last-fetch`;
